...
LOG_MODULE_REGISTER(inv_mpu9250_log, LOG_LEVEL_DBG);
...
extern int mpu9250_i2c_write(unsigned char slave_addr, unsigned char reg_addr,
                       unsigned char length, unsigned char * data);

extern int mpu9250_i2c_read(unsigned char slave_addr, unsigned char reg_addr,
                       unsigned char length, unsigned char * data);

extern void mpu9250_delay_ms(unsigned long ms);

extern void mpu9250_get_ms(unsigned long *count);

#define i2c_write(a, b, c, d) mpu9250_i2c_write(a, b, c, d)
#define i2c_read(a, b, c, d)  mpu9250_i2c_read(a, b, c, d)
#define delay_ms(t)  mpu9250_delay_ms(t)
#define get_ms(t)    mpu9250_get_ms(t)
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    int err = i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data);
    if (err != 0) {
        LOG_ERR("MPU9250 reset failed!");
        return -1;
    }
    delay_ms(100);
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data)) {
        LOG_ERR("Setting MPU9250 fifo size failed!");
        return -1;
    }
#endif
    ...
    if (mpu_set_gyro_fsr(2000)) {
        LOG_ERR("Setting MPU9250 gyro fsr failed!");
        return -1;
    }
    if (mpu_set_accel_fsr(2)) {
        LOG_ERR("Setting MPU9250 accel fsr failed!");
        return -1;
    }
    if (mpu_set_lpf(42)) {
        LOG_ERR("Setting MPU9250 lowpass filter failed!");
        return -1;
    }
    if (mpu_set_sample_rate(50)) {
        LOG_ERR("Setting MPU9250 sample rate failed!");
        return -1;
    }
    if (mpu_configure_fifo(0)) {
        LOG_ERR("Configuring MPU9250 fifo buffer failed!");
        return -1;
    }
    ...
    if (mpu_set_compass_sample_rate(10)) {
        LOG_ERR("Setting MPU9250 magn sample rate failed!");
        return -1;
    }
#else
    /* Already disabled by setup_compass. */
    if (mpu_set_bypass(0)) {
        LOG_ERR("Setting MPU9250 bypass mode to 0 failed!!");
        return -1;
    }
#endif
